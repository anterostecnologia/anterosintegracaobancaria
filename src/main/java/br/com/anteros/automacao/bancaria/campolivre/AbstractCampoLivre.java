...
abstract class AbstractCampoLivre extends BlockOfFields implements CampoLivre {
	 ...
	 * Looger.
	 */
	private static Logger log = LoggerProvider.getInstance().getLogger(AbstractCampoLivre.class);
	...
	private static void checkContaBancariaNotNull(Titulo titulo) {
		...
		Assert.notNull(titulo.getContaBancaria(), "Conta bancária do título não pode ser nula!");
	}
	...
	private static void checkBancoNotNull(Titulo titulo) {
		...
		Assert.notNull(titulo.getContaBancaria().getBanco(), "Banco da conta bancária do título não pode ser nulo!");
		...
		Assert.checkArgument(expression, format("Código de compensação [%s] inválido!", titulo.getContaBancaria().getBanco().getCodigoDeCompensacaoBACEN().getCodigoFormatado()));
	}
	...
	protected final static void checkCarteiraNotNull(Titulo titulo){
		...
		Assert.notNull(titulo.getCarteira(), "Carteira da conta bancária do título não pode ser nula!");
	}
	...
	protected final static void checkRegistroDaCarteiraNotNull(Titulo titulo){
		...
		Assert.notNull(titulo.getCarteira().getTipoDeCobranca(), "Tipo de cobrança (COM ou SEM registro) da carteira não pode ser nulo!");
	}
	...
	protected final static void checkCodigoDaCarteira(Titulo titulo){
		...
		Assert.notNull(titulo.getCarteira().getCodigo(), "Código da carteira não pode ser nulo!");
		
		boolean expression = titulo.getCarteira().getCodigo() > 0; 
		
		Assert.checkArgument(expression, format("Código da carteira deve ser um número inteiro natural positivo e não [%s].",titulo.getCarteira().getCodigo()));
	}
	...
	protected final static void checkCodigoDaCarteiraMenorOuIgualQue(Titulo titulo, int limite){
		...
		boolean expression = titulo.getCarteira().getCodigo() <= limite;
		
		Assert.checkArgument(expression,format("Código [%s] da carteira deve ser um número menor que ou igual a [%s].", titulo.getCarteira().getCodigo(), limite));
	}
	...
	protected final static void checkAgenciaNotNull(Titulo titulo){
		...
		Assert.notNull(titulo.getContaBancaria().getAgencia(), "Agência bancária do título não pode ser nula!");
	}
	...
	protected final static void checkCodigoDaAgencia(Titulo titulo){
		...
		Assert.notNull(titulo.getContaBancaria().getAgencia().getCodigo(), "Código da agência bancária não pode ser nulo!");
		
		boolean expression = titulo.getContaBancaria().getAgencia().getCodigo() > 0; 
		
		Assert.checkArgument(expression, format("Código da agência bancária deve ser um número inteiro natural positivo e não [%s].",titulo.getContaBancaria().getAgencia().getCodigo()));
	}
	...
	protected final static void checkCodigoDaAgenciaMenorOuIgualQue(Titulo titulo, int limite){
		...
		Assert.checkArgument(expression , format("Código [%s] da agência deve ser um número menor que ou igual a [%s].", titulo.getContaBancaria().getAgencia().getCodigo(), limite));
	}
	...
	protected final static void checkDigitoDoCodigoDaAgencia(Titulo titulo){
		...
		Assert.notNull(titulo.getContaBancaria().getAgencia().getDigitoVerificador(), "Dígito verificador da agência bancária não pode ser nulo!");
		Assert.notNull(titulo.getContaBancaria().getAgencia().getDigitoVerificador(), format("Dígito verificador [\"%s\"] da agência bancária não pode ser vazio!",titulo.getContaBancaria().getAgencia().getDigitoVerificador()));
		StringUtils.checkNotNumeric(titulo.getContaBancaria().getAgencia().getDigitoVerificador(), format("Nesse contexto o dígito verificador [\"%s\"] da agência bancária deve ser numérico!", titulo.getContaBancaria().getNumeroDaConta().getDigitoDaConta()));
	}
	...
	protected final static void checkNumeroDaContaNotNull(Titulo titulo){
		...
		Assert.notNull(titulo.getContaBancaria().getNumeroDaConta(), "Número da conta bancária do título não pode ser nulo!");
	}
	...
	protected final static void checkCodigoDoNumeroDaConta(Titulo titulo){
		...
		Assert.notNull(titulo.getContaBancaria().getNumeroDaConta().getCodigoDaConta(), "Código do número da conta bancária não pode ser nulo!");
		
		boolean expression = titulo.getContaBancaria().getNumeroDaConta().getCodigoDaConta() > 0;
		
		Assert.checkArgument(expression, format("Código do número da conta bancária deve ser um número inteiro natural positivo e não [%s].", titulo.getContaBancaria().getNumeroDaConta().getCodigoDaConta()));
	}
	...
	protected final static void checkCodigoDoNumeroDaContaMenorOuIgualQue(Titulo titulo, int limite){
		...
		Assert.checkArgument(expression, format("Código [%s] do número da conta deve ser um número menor que ou igual a [%s].", titulo.getContaBancaria().getNumeroDaConta().getCodigoDaConta(), limite));
	}
	...
	protected final static void checkDigitoDoCodigoDoNumeroDaConta(Titulo titulo){
		...
		Assert.notNull(titulo.getContaBancaria().getNumeroDaConta().getDigitoDaConta(), "Dígito verificador do número da conta bancária não pode ser nulo!");
		Assert.notNull(titulo.getContaBancaria().getNumeroDaConta().getDigitoDaConta(), format("Dígito verificador [\"%s\"] do número da conta bancária não pode ser vazio!", titulo.getContaBancaria().getNumeroDaConta().getDigitoDaConta()));
		StringUtils.checkNotNumeric(titulo.getContaBancaria().getNumeroDaConta().getDigitoDaConta(), format("Nesse contexto o dígito verificador [\"%s\"] do número da conta deve ser numérico!", titulo.getContaBancaria().getNumeroDaConta().getDigitoDaConta()));
	}
	...
	protected final static void checkNossoNumero(Titulo titulo){
		...
		Assert.notNull(titulo.getNossoNumero(), "Nosso número do título não pode ser nulo!");
		Assert.notNull(titulo.getNossoNumero(), format("Nosso número [\"%s\"] do título não pode ser vazio!", titulo.getNossoNumero()));
		StringUtils.checkNotNumeric(titulo.getNossoNumero(), format("Nosso número [\"%s\"] do título deve conter somente dígitos numéricos!", titulo.getNossoNumero()));
	}
	...
	protected final static void checkTamanhoDoNossoNumero(Titulo titulo, int length) {
		...
		checkTamanhoNossoNumero(titulo, length, format(
				...
				titulo.getNossoNumero().length(), titulo.getNossoNumero(), length));
	}
	...
	protected final static void checkTamanhoNossoNumero(Titulo titulo, int length, String msg){
		...
		Assert.checkArgument(titulo.getNossoNumero().length() == length, msg);
	}
	...
	protected final static void checkParametroBancario(Titulo titulo, ParametroBancario<?> param){
		...
		Assert.notNull(titulo.getParametrosBancarios(), format("O parâmetro bancário [\"%s\"] é necessário! [titulo.getParametrosBancarios() == null]",param));
		Assert.checkArgument(titulo.getParametrosBancarios().contemComNome(param),format("Parâmetro bancário [\"%s\"] não encontrado!",param));
		Assert.notNull(titulo.getParametrosBancarios().getValor(param), format("Parâmetro bancário [\"%s\"] não contém valor!", param));
	}
	...
	protected final static void checkParametroBancarioMenorOuIgualQue(Titulo titulo, ParametroBancario<?> param, int limite){
		...
		Assert.checkArgument(expression, format("Parâmetro [%s] com valor [%s] deve ser um número menor que ou igual a [%s].", param, valor, limite));
	}
