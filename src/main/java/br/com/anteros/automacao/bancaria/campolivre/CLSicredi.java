...
class CLSicredi extends AbstractCLSicredi {
	...
	protected void checkValues(Titulo titulo){
		...
		checkParametroBancario(titulo, ParametroBancoSicredi.POSTO_DA_AGENCIA);
	...
	protected void addFields(Titulo titulo) {
		...
		if(titulo.getCarteira().isComRegistro()){
		...
		this.add(new FixedField<Long>(titulo.getContaBancaria().getNumeroDaConta().getCodigoDaConta(), 5, Fillers.ZERO_LEFT));
	...
	private void checkCarteiraSimples(Titulo titulo) {
		...
		Assert.checkArgument(
				titulo.getCarteira().getCodigo().equals(CARTEIRA_SIMPLES_VALUE),
				format("Apenas a carteira de código [1] \"carteira simples\" é permitida e não o código [%s]!", titulo.getCarteira().getCodigo())
		);
	}
