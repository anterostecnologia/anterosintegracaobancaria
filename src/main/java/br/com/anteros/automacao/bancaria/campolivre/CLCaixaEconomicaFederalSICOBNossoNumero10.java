...
class CLCaixaEconomicaFederalSICOBNossoNumero10 extends AbstractCLCaixaEconomicaFederal {
	...
	CLCaixaEconomicaFederalSICOBNossoNumero10(Titulo titulo) {
		...
		Assert.notNull(titulo.getParametrosBancarios(), "Parâmetros bancários necessários [titulo.getParametrosBancarios()==null]!");
		...
		*/
		...
		if(titulo.getParametrosBancarios().contemComNome(ParametroCaixaEconomicaFederal.CODIGO_OPERACAO)){
			...
			Assert.notNull(titulo.getParametrosBancarios(), "Parâmetro bancário código operação inválido [CodigoOperacao==null]!");
	...
	private void checkPadraoNossoNumero(String nn){
		if(!nn.startsWith("3") && !nn.startsWith("9") && !nn.startsWith("80") && !nn.startsWith("81") && !nn.startsWith("82")){
			throw new IllegalArgumentException(format("Para a cobrança CEF o nosso número [%s] deve começar com 3 que é o identificador da \"carteira siples\" [3NNNNNNNNN] ou 9 que é o identificador da \"carteira rápida\" [9NNNNNNNNN] ou 80, 81 e 82 para \"carteira sem registro\" [82NNNNNNNN]!", nn));
		}
	}
	...
	private void checkPadraoNossoNumeroPorCodigoDaCarteira(String nossoNumero, Integer codigoDaCarteira){
		...
		case 11:
			if(!nossoNumero.startsWith("3")){
				throw new IllegalArgumentException(format("Para a cobrança CEF, carteira 11 (cobrança simples), o nosso número [%s] deve começar com 3!", nossoNumero));
			}
			break;

		case 12:
			if(!nossoNumero.startsWith("9")){
				throw new IllegalArgumentException(format("Para a cobrança CEF, carteira 12 (cobrança rápida), o nosso número [%s] deve começar com 9!", nossoNumero));
			}
			break;
					
		case 14:
			if(!nossoNumero.startsWith("80") && !nossoNumero.startsWith("81") && !nossoNumero.startsWith("82")){
				throw new IllegalArgumentException(format("Para a cobrança CEF, carteira 14 (cobrança sem registro), o nosso número [%s] deve começar com 80, 81 ou 82!", nossoNumero));
			}
			break;
		}
	}
