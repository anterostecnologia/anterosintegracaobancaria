...
import br.com.anteros.automacao.bancaria.parametro.ParametroBancoDeBrasilia;
...
public class CLBancoDeBrasilia extends AbstractCLBancoDeBrasilia{
	...
	private void checkCarteiraComOuSemRegistro(Titulo titulo) {
		...
		Integer codigoCarteira = titulo.getCarteira().getCodigo();
		
		final boolean carteiraValida = codigoCarteira.equals(CARTEIRA_SEM_REGISTRO) || codigoCarteira.equals(CARTEIRA_COM_REGISTRO);
		
		Assert.checkArgument(carteiraValida, format("Código da carteira deve ser \"1-Sem registro impressão local\" ou  \"2-Com registro impressão local\" e não [%s].", titulo.getCarteira().getCodigo()));
	}
