package br.com.anteros.integracao.bancaria.banco.layout.cnab400.cobranca.santander;
...
public class SantanderCNAB400 implements CNAB400 {
	...
	@Record(name = TRAILLER_REMESSA, description = "Protocolo de comunicação", order = 1, groups = { REMESSA_COBRANCA })
	...
	@Record(name = HEADER_RETORNO, description = "Protocolo de comunicação", order = 1, groups = { RETORNO_COBRANCA })
	...
	@Record(name = TRAILLER_RETORNO, description = "Resumo do arquivo de Retorno", order = 1, groups = { RETORNO_COBRANCA })
	private TraillerArquivoRetorno traillerArquivoRetorno;
	private List<RemessaCobranca> remessas;

	public SantanderCNAB400(ContaBancaria contaBancaria, List<RemessaCobranca> remessas, Date dataHoraGeracao,
			Date dataGravacao) {
		...
		headerArquivoRemessa = HeaderArquivoRemessa.of(contaBancaria,
				...
				remessas.get(PRIMEIRA_REMESSA).getTitulo().getCedente(), dataGravacao, dataHoraGeracao);
	...
	public SantanderCNAB400(ContaBancaria contaBancaria, Date dataHoraGeracao, Date dataGravacao) {
		headerArquivoRetorno = HeaderArquivoRetorno.of(contaBancaria);
		detalheRetorno = TitulosCobrancaDetalheRetorno.of(contaBancaria);
		traillerArquivoRetorno = TraillerArquivoRetorno.of(contaBancaria);
	}
