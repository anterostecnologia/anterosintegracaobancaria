...
public class CobrancaSegmentoQ implements RecordData {
	...
	@Field(name = CD_BANCO_CORRESPONDENTE_COMPE, length = 3, type = EnumTypes.INTEGER, padding = Paddings.ZERO_LEFT, value="000")
	private Integer codigoBancoCorrespondenteCompe;

	@Field(name = NOSSO_NUMERO_BANCO_CORRESPONDENTE, length = 20, padding = Paddings.WHITE_SPACE_RIGHT, value=" ")
	...
	public void readRowData(int row, int sequence) {
		...
		this.endereco = remessas.get(row).getTitulo().getSacado().getNextEndereco().getLogradouro() + ", " + remessas.get(row).getTitulo().getSacado().getNextEndereco().getNumero();// G032
