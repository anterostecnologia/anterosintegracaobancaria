...
public class TraillerLote {
	...
	@Field(name = VL_TOTAL, length = 18, type = EnumTypes.BIGDECIMAL, padding = Paddings.ZERO_LEFT, format = Formats.DECIMAL_DD)
	private BigDecimal valorTotal;
	
	@Field(name = QT_MOEDA, length = 18, type = EnumTypes.BIGDECIMAL, padding = Paddings.ZERO_LEFT, format = Formats.DECIMAL_DDDDD)
	private BigDecimal totalQuantidadeMoedas;
	
	@Field(name = NR_AVISO, length = 6, type = EnumTypes.INTEGER, padding = Paddings.ZERO_LEFT)
	private Integer numeroAvisoDebito;

	@Field(name = BRANCOS_2, length = 165, value = " ", padding = Paddings.WHITE_SPACE_LEFT)
	...
	public TraillerLote(ContaBancaria contaBancaria, List<RemessaCobranca> remessas) {
	...
	public TraillerLote(ContaBancaria contaBancaria) {
		this.codigoBanco = contaBancaria.getBanco().getCodigoDeCompensacaoBACEN().getCodigo(); // G001
		this.loteServico = 1;// G002
	}

	public static TraillerLote of(ContaBancaria contaBancaria, List<RemessaCobranca> remessas) {
		return new TraillerLote(contaBancaria, remessas);
	}
	...
	public static TraillerLote of(ContaBancaria contaBancaria) {
		return new TraillerLote(contaBancaria);
	}
