...
public class TitulosCobrancaSegmentoP implements RecordData {

	@IdType(name = "CD_SEGMENTO_REGISTRO", length = 1, position = 5, value = "P")
	private String codigoSegmentoRegistro;

	@Field(name = "CD_BANCO", length = 3, value = "1", type = EnumTypes.INTEGER, padding = Paddings.ZERO_LEFT)
	...
	@Field(name = "NR_CONTACORRENTE", length = 12, type = EnumTypes.INTEGER)
	...
	public TitulosCobrancaSegmentoP(ContaBancaria contaBancaria, List<RemessaCobranca> remessas) {
	...
	public void readRowData(int row) {
		...
		this.numeroDocumentoCobranca = remessas.get(row).getTitulo().getNumeroDoDocumento();//C011
		this.dtVencimentoTitulo = remessas.get(row).getTitulo().getDataDoVencimento();//C012
		this.vlNominalTitulo = remessas.get(row).getTitulo().getValor();//G070
		...
		this.especieTitulo = remessas.get(row).getTitulo().getTipoDeDocumento().getCodigo();//C015
		this.identificadorTituloAceito = remessas.get(row).getTitulo().getAceite().name();//C016
		this.dtEmissaoTitulo = remessas.get(row).getTitulo().getDataDoDocumento(); //G071
		this.codigoJurosMora = remessas.get(row).getTitulo().getTipoDeJurosMora().getTipo();//C018
		this.dtJurosMora = remessas.get(row).getTitulo().getDataJurosMora(); //C019
		this.jurosMoraDia = remessas.get(row).getTitulo().getValorJurosMora(); //C020
		this.codigoDesconto1 = remessas.get(row).getTitulo().getTipoDeDesconto().getTipo();//C021
		...
		this.identificadorTituloEmpresa = remessas.get(row).getTitulo().getNumeroDoDocumento();//G072
		this.codigoProtesto = remessas.get(row).getTitulo().getTipoDeProtesto().getTipo();//C026
		...
		this.codigoMoeda = remessas.get(row).getTitulo().getTipoDeMoeda().getCodigo();//G065
	...
	public static TitulosCobrancaSegmentoP of(ContaBancaria contaBancaria, List<RemessaCobranca> remessas) {
